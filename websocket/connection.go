...
func newConnection(ctx *gin.Context, s *Server, underlineConn UnderlineConnection, id string) *connection {
	...
	if s.config.Authentication {
		...
		if len(token) > 0 {
			userJSon, err := redis.Cluster.Get(redis.Cluster.Context(), consts.RedisTokenPrefix+token).Result()
			if err == nil {
				var user = consts.User{}
				data := consts.JsonStringConvert([]byte(userJSon))
				if err := json.Unmarshal(data, &user); err != nil {
					log.Printf("redisToken转换json出错%s,%v", userJSon, err)
				}
				c.uid = strconv.Itoa(user.Id)
			}
		}
	}
...
func (c *connection) WriteDefault(data []byte) error {
	if c.server.config.GzipContent {
		if val, err := GzipEncode(data); err == nil {
			data = val
		}
	}
...
func (c *connection) messageEventHandle(data []byte) {
	data = bytes.TrimSpace(bytes.Replace(data, newline, space, -1))
	if bytes.EqualFold([]byte(consts.PingMessageText), data) || bytes.EqualFold([]byte(consts.PongMessageText), data) {
		c.Write(websocket.TextMessage, []byte("PONG"))
		return
	}
	var subMsg consts.SubMsg
	e := json.Unmarshal(data, &(subMsg))
	if e != nil {
		//bytes.Replace("")
		log.Println("json转换出错 data:", string(data))
		return
	}
	listeners, ok := c.onEventListeners[subMsg.Event]
	if !ok || len(listeners) == 0 {
		return //未注册事件
	}
	for i := range listeners {
		if fnSubMsg, ok := listeners[i].(func(*consts.SubMsg)); ok {
			fnSubMsg(&subMsg)
		}
	}
}
